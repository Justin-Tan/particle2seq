...
class Data(object):
    ...
    def load_data(filename, evaluate=False, adversary=False):
        ...
        if evaluate:
            ...
            if adversary:
                ...
                return np.nan_to_num(df_features.values), df['label'].values, pivot_df.values.astype(np.float32)
            else:
    ...
    @staticmethod
    def load_dataset_adversary(features_placeholder, labels_placeholder, batch_size, test=False, evaluate=False):
