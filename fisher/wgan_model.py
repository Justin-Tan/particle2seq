...
class Model():
    def __init__(self, config, features, labels, args, evaluate=False):
        ...
        self.disc_step = tf.Variable(0, trainable=False)
        ...
        with tf.variable_scope('Discriminator') as scope:
            ...
            Z_prime_bkg = tf.boolean_mask(Z_prime, tf.cast(1-self.labels, tf.bool))
            # Z_prime_bkg = tf.random_shuffle(Z_bkg) 
            Z_bkg, Z_prime_bkg = tf.expand_dims(Z_bkg, axis=1), tf.expand_dims(Z_prime_bkg, axis=1)

            # Test - project Z into same dimension as E
            dim = 512
            Z_bkg.set_shape([None, 1])
            Z_prime_bkg.set_shape([None, 1])
            print('VLAD', Z_bkg.shape)
            Z_bkg = tf.layers.dense(Z_bkg, units=dim)
            Z_prime_bkg = tf.layers.dense(Z_prime_bkg, units=dim)

            self.wasserstein_metric, wasserstein_disc_cost = Network.wasserstein_distance(x=X_bkg, y=Z_bkg,
                    y_prime=Z_prime_bkg, labels=self.labels, batch_size=config.batch_size, dimension=dim, training=True, actv=tf.nn.elu,
                    gradient_penalty=True, lambda_gp=10.)
        ...
        if args.mutual_information_penalty:
            ...
            self.cost += args.MI_lambda * tf.nn.relu(self.wasserstein_metric)
        ...
        with tf.control_dependencies(update_ops):
            ...
            disc_opt = tf.train.AdamOptimizer(config.D_learning_rate, beta1=0.5, beta2=0.9)

            self.disc_opt_op = disc_opt.minimize(self.disc_cost, var_list=theta_disc,
                    global_step=self.disc_step)

            self.MINE_labels_opt_op = disc_opt.minimize(-self.MINE_labels_lower_bound, var_list=theta_MINE_NY)
            
            self.ema = tf.train.ExponentialMovingAverage(decay=config.ema_decay, num_updates=self.global_step)
            self.disc_ema = tf.train.ExponentialMovingAverage(decay=0.95, num_updates=self.disc_step)
            maintain_averages_clf_op = self.ema.apply(theta_f)
            maintain_averages_disc_op = self.disc_ema.apply(theta_disc)
            ...
            with tf.control_dependencies(update_ops+[self.disc_opt_op]):
                self.disc_train_op = tf.group(maintain_averages_disc_op)
