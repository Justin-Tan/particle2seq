...
class Model():
    def __init__(self, config, features, labels, args, evaluate=False):
        ...
        if evaluate:
            ...
            self.log_likelihood = -tf.reduce_sum(self.cross_entropy)
        ...
        self.softmax = tf.squeeze(tf.nn.sigmoid(self.logits))
        self.pred = tf.cast(tf.greater(self.softmax, 0.5), tf.int32)
        ...
        if config.use_adversary:
            ...
            with tf.variable_scope('mmd') as scope:
                ...
                self.mmd2_loss = Network.kernel_MMD(x=X_bkg, y=Z_bkg, y_prime=Z_prime_bkg, batch_size=config.batch_size,
                        dimension=2, labels=self.labels, bkg_only=True)
            ...
            theta_mmd = Utils.scope_variables('mmd')
            print('Classifier parameters:', theta_f)
            print('mine parameters', theta_mmd)
            
            if args.mmd_penalty:
                print('Penalizing kernel mmd')
                self.cost += args.mmd_lambda * tf.nn.relu(self.mmd2_loss)
        ...
        tf.summary.scalar('mmd2', self.mmd2_loss)
