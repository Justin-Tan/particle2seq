...
if __name__ == '__main__':
    ...
    if args.smoke_test:
        ...
        mnist_spec['stop']['time_total_s'] = 600

    ray.init()
    hyperband = HyperBandScheduler(
        time_attr="training_iteration", reward_attr="mean_accuracy", max_t=10)

    trials = run_experiments({'mnist_hyperband_test_test': mnist_spec}, scheduler=hyperband)
    t_ids = [t.trial_id for t in trials]
    t_config = [t.config for t in trials]
    t_result = [t.last_result for t in trials]
    g = [t_ids, t_config, t_result]
    df = pd.DataFrame(g)
    df = df.transpose()
    df.columns = ['name', 'config', 'result']
    best = get_best_trial(trials, metric="mean_accuracy")
    print(best)
    print(type(best))
    print(g)
    json.dump(g, open('pie.json', 'w'))

    df.to_hdf('pies.h5', key='df')
