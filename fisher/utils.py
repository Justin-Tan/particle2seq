...
class Utils(object):
    ...
    @staticmethod
    def run_mmd_diagnostics(model, config, directories, sess, saver, train_handle,
            test_handle, start_time, v_auc_best, block, step, name):
        t0 = time.time()
        improved = ''
        sess.run(tf.local_variables_initializer())
        ...
        v_MI_kraskov, v_auc, v_acc, v_loss, v_mmd_loss, v_summary, y_true, y_pred = sess.run([model.MI_logits_theta_kraskov, 
            model.auc_op, model.accuracy, model.cost, model.mmd2_loss,
            model.merge_op, model.labels, model.pred], feed_dict=feed_dict_test) # TEST
        model.test_writer.add_summary(v_summary)
        ...
        print("Block {} | Test Acc: {:.3f} | Train auc: {:.3f} | Test auc: {:.3f} | MI_kraskov: {:.3f} | " 
                "mmd_loss: {:.3f} | Train Loss: {:.3f} | Test Loss: {:.3f} | "
                "Rate: {} examples/s ({:.2f} s) {}".format(block, v_acc, t_auc, v_auc, v_MI_kraskov, v_mmd_loss, 
                    t_loss, v_loss, int(config.batch_size/(time.time()-t0)), time.time() - start_time, improved))
        Utils.plot_distributions(model, name, block, step, sess, handle=test_handle, path='/home/jtan/gpu/jtan/github/fisher')
